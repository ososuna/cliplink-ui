...
export class ServerAuthRepository implements IAuthRepository {
  ...
  async login(data: LoginRequest): Promise<Result<User>> {
    try {
      const user = await this.httpClient.post<User>('/auth/login', data);
      return success(user);
    } catch (e) {
  ...
  async register(data: RegisterRequest): Promise<Result<User>> {
    try {
      const user = await this.httpClient.post<User>('/auth/register', data);
      return success(user);
    } catch (e) {
  ...
  async checkToken(token?: string): Promise<Result<User>> {
    try {
      const user = await this.httpClient.get<User>('/auth/token');
      return success(user);
    } catch (e) {
  ...
  async refreshToken(token?: string): Promise<Result<User>> {
    try {
      const user = await this.httpClient.get<User>('/auth/refresh-token');
      return success(user);
    } catch (e) {
  ...
  async logout(): Promise<Result<void>> {
    try {
      await this.httpClient.get<void>('/auth/logout');
  ...
  async update(data: UpdateUserRequest): Promise<Result<User>> {
    try {
      const user = await this.httpClient.put<User>('/auth', data);
      return success(user);
    } catch (e) {
  ...
  async authGithub(): Promise<Result<void>> {
    try {
      await this.httpClient.get<void>('/auth/github');
      return success(undefined);
    } catch (e) {
  ...
  async deleteAccount(): Promise<Result<void>> {
    try {
      await this.httpClient.delete<void>('/auth');
      return success(undefined);
    } catch (e) {
  ...
  async forgotPassword(email: string): Promise<Result<void>> {
    try {
      await this.httpClient.post<void>('/auth/forgot-password', { email });
      return success(undefined);
    } catch (e) {
  ...
  async checkPasswordToken(token: string): Promise<Result<ResetPasswordToken>> {
    try {
      const resetToken = await this.httpClient.get<ResetPasswordToken>(`/auth/password-token/${token}`);
  ...
  async updatePassword(token: string, password: string): Promise<Result<User>> {
    try {
      const user = await this.httpClient.put<User>('/auth/update-password', { token, password });
      return success(user);
    } catch (e) {
